...
import { collection, getDocs, increment, limit, orderBy, query, updateDoc, where } from 'firebase/firestore';
...
export const getFriendsTopUsersByScore = async (currentUserId: string) => {
    try {
        ...
        const q2 = query(usersCollectionRef, where('uid', 'in', friendsUid), orderBy('score', 'desc'), limit(100));
